...
case "$agent" in
    ...
    jarvis) profile="jarvis"; model="gpt-5.6-sol"; effort="high"; greeting="Jarvis at your service." ;;
    ultron) profile="ultron"; model="gpt-6-astra"; effort="medium"; greeting="Lowly human, let Ultron manage the rest." ;;
...
printf '%s\n' "$greeting"
exec codex --profile "$profile" --model "$model" --config "model_reasoning_effort=\"$effort\"" $live_search_flag $full_access_flag $sandbox_mode_args "$@"
