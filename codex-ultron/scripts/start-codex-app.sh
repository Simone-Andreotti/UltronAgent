...
case "$agent" in
    edith) model="gpt-5.6-luna"; instructions_file="edith.md" ;;
    jarvis) model="gpt-5.6-terra"; instructions_file="jarvis.md" ;;
    ultron) model="gpt-5.6-sol"; instructions_file="ultron.md" ;;
...
exec codex app --config "model=\"$model\"" --config 'model_reasoning_effort="high"' --config 'model_verbosity="low"' --config "$developer_instructions" "$working_directory"