...
developer_instructions=$(printf 'developer_instructions="""\n%s\n"""' "$(cat "$package_root/instructions/ultron.md")")

exec codex app --config 'model="gpt-5.6-sol"' --config 'model_reasoning_effort="high"' --config 'model_verbosity="low"' --config "$developer_instructions" "${1:-$PWD}"