...
2. Before planning or implementation for non-trivial, ambiguous, multi-file, or architectural work, first spawn exactly one bounded read-only `luna-code-analyst` task to inspect only files and symbols needed for the assigned task or question; simple local fixes may stay direct. Use `luna-code-analyst` only for that bounded task-scoped inspection, and `luna-researcher` only for unresolved external evidence. Use `luna-worker` for routine implementation, focused checks, browser tests, screenshots, and image elaboration.
