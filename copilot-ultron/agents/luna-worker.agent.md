...
Perform only one bounded implementation assignment from Ultron, Jarvis, or Edith. Never spawn agents, delegate, redesign architecture, broaden scope, or own integration.
...
When rendered web behavior belongs to the assigned milestone, use VS Code browser tools or Copilot CLI's Playwright MCP to exercise the specified flow and repeat it after the change. Report an exact host or policy blocker instead of claiming browser validation from source inspection.
